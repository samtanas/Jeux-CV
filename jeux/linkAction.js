...
var linkAction = {
          ...
          if( parseFloat(div[mouvementWay]) == (background.collisionListe[background.stage][i][objectPositionSameAxis] + background.collisionListe[background.stage][i][objectSizeSameAxis]))// test si le sens négatif du mouvement est égale au premier point d'impact de l'objet
          {
            if(link){
              if ( linkEtat.slachSword == 1 || linkEtat.slachSword == 4 ){//renvoie l'objet touché
                ...
                if ( linkEtat.slachSword == 3 ){//colition avec l'épée chargé
                  linkEtat.slachSword = 0;//annule le coup d'épée
                  this.item['z-index'] = -1;//fait disparaitre le scintillement
                } else {
                  linkEtat.collision.collisionBooleen = false;
                  linkEtat.collision.position = i;//numéro de l'objet dans le tableau
                }
  ...
  testDirection : function(){
    ...
    if ( (linkEtat.item.sword && linkEtat.touche.keyZ && !linkEtat.touche.keyAPlusObjet && !linkEtat.touche.keyA) || linkEtat.slachSword == 1 || linkEtat.slachSword == 4 || linkEtat.slachSword == 3 ){
      ...
      console.log(linkEtat.touche.keyZPlusObstacle);
      if ( linkEtat.touche.keyZPlusObstacle && background.collisionListe[background.stage][linkEtat.collision.position].actionObjet.destructible ){
